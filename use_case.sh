...
curl -s ${path_for_movies} | ./json_pretty.sh > movies.txt
cat movies.txt
...
curl -s ${path_for_rooms} | ./json_pretty.sh > rooms.txt
cat rooms.txt
...
# remember to specify path to `jq` to get user id
path_for_user="localhost:8080/user/"${name}"/"${surname}
curl -s ${path_for_user} | ./json_pretty.sh |  ../jq-win64.exe -r '.id' > user.txt
